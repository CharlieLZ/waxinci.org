...
def get_ready_tasks(max_calls=10):
    """获取所有已完成任务列表"""
        ...
        try:
            ...
            for entry in tasks_list:
                ...
                if isinstance(ready_tasks, list) and ready_tasks:
                    total_found += len(ready_tasks)
                    all_ready_tasks.extend(ready_tasks)
                    
            if total_found > 0:
                logger.info(f"本次API调用发现 {total_found} 个已完成的任务")
                # 如果这次拉取数量不足最大页大小，说明已取完全部历史任务
                if total_found < 1000:
                    logger.info(f"已获取所有可用的完成任务，共 {len(all_ready_tasks)} 个")
                    break
            else:
                logger.info("当前无已完成任务或 result 不是列表")
                break
...
def process_tasks_batch(task_ids, max_workers=10):
    """并发处理多个已完成的任务"""
    results = {}
...
def main():
    """主函数 - 从历史任务中获取数据"""
    logger.info("开始运行谷歌趋势历史查询脚本")
    
    try:
        # 获取所有已完成任务
        logger.info("获取所有已完成任务...")
        ready_tasks = get_ready_tasks(max_calls=10)
        ...
        # 限制处理数量以节省成本（可以根据需要调整）
        max_tasks = 50  # 只处理前50个任务
        if len(task_ids) > max_tasks:
            task_ids = task_ids[:max_tasks]
            logger.info(f"为节省成本，只处理前 {max_tasks} 个任务")
        
        # 并发处理任务
        logger.info(f"开始并发处理 {len(task_ids)} 个任务...")
        results = process_tasks_batch(task_ids, max_workers=10)
        
        if results:
            # 保存数据
            success = save_website_data(results)
            if success:
                logger.info(f"历史查询完成，共处理 {len(results)} 个关键词")
            else:
                logger.error("数据保存失败")
        else:
